...
public class ProjectImageValidator {
    ...
    public static Optional<String> validateUpload(final String contentType, final byte[] body, final String altText) {
        if (!validProjectImageContentType(contentType)) {
            return Optional.of(PROJECT_IMAGE_TYPE_INVALID_CODE);
        }
        if (body == null || body.length == 0 || body.length > PROJECT_IMAGE_MAX_BYTES) {
            return Optional.of(PROJECT_IMAGE_TOO_LARGE_CODE);
        }
        if (!validProjectImageBytes(contentType, body)) {
            return Optional.of(PROJECT_IMAGE_TYPE_INVALID_CODE);
        }
        if (altText != null && altText.length() > PROJECT_IMAGE_ALT_TEXT_MAX_LENGTH) {
            return Optional.of(PROJECT_IMAGE_ALT_TEXT_INVALID_CODE);
        }
